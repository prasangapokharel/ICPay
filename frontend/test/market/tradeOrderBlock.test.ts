...
assert(tradeOrderBlock({
  tradingBal: 100n, amountIn: 50n, maxIn: 90n, aboveMinIcp: false, hasQuote: true,
}) === "min_icp", "under min icp")
...
assert(tradeOrderAlert("no_pool", 0.05)?.key === "noPoolHint", "pool alert")
assert(tradeOrderAlert("no_liquidity", 0.05)?.destructive === true, "liquidity destructive")
assert(tradeOrderAlert("min_icp", 0.05)?.values?.icp === 0.05, "min icp")
assert(tradeOrderAlert("min_usd", 0.05)?.values?.usd === 1, "min usd")
assert(tradeOrderAlert("quote_error", 0.05, "unsupported")?.key === "unsupportedToken", "unsupported")
assert(tradeOrderAlert("insufficient", 0.05) === null, "balance lives on the button")
assert(tradeOrderAlert("ok", 0.05) === null, "no alert when ready")

...
export async function listCommunityMessages(
  ...
  return query(identity, async (actor) => {
    ...
    >
    return unwrap(result).map(normalizeCommunityMessage)
  })
}
...
export async function getPublicCommunityChannel(slug: string): Promise<CommunityChannelPublic | null> {
  try {
    const actor = await getWalletActor(undefined)
    const r = (await actor.getCommunityChannel(slug)) as [] | [CommunityChannelPublic]
    return r.length ? normalizeCommunityChannel(r[0]) : null
  } catch {
    return null

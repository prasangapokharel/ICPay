...
export async function joinLiveRoom(
  ...
  const outcome = await call(identity, 'Could not join room', async (actor) =>
    actor.joinLiveRoom(roomId, tabId, inviteToken ? [inviteToken] : []) as Promise<Outcome<LiveRoomPublic>>,
  )
  return unwrap(outcome)
}
...
export async function endLiveRoom(identity: Identity | undefined, roomId: string): Promise<void> {

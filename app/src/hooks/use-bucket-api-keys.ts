...
import { createApiKey, listApiKeys, revokeApiKey } from '@/services/bucket/bucket'
import type { ApiKeyCreateResult, ApiKeyPermissions, ApiKeyPublic } from '@/services/bucket/types'
...
export function useBucketApiKeys(bucketId: string | null, enabled = true) {
  ...
  return {
    ...
    isLoading,
    refresh: mutate,
    createKey: (name: string, permissions: ApiKeyPermissions): Promise<Outcome<ApiKeyCreateResult>> =>

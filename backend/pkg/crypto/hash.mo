...
module {
  ...
  public func sha256(data: [Nat8]) : [Nat8] {
    ...
    for (i in data.keys()) { padded[i] := data[i] };
    ...
    var h0v : Nat32 = 0x6a09e667;
    var h1v : Nat32 = 0xbb67ae85;
    var h2v : Nat32 = 0x3c6ef372;
    var h3v : Nat32 = 0xa54ff53a;
    var h4v : Nat32 = 0x510e527f;
    var h5v : Nat32 = 0x9b05688c;
    var h6v : Nat32 = 0x1f83d9ab;
    var h7v : Nat32 = 0x5be0cd19;

    let w = VarArray.repeat<Nat32>(0 : Nat32, 64);
    let blk = VarArray.repeat<Nat8>(0, 64);
    var pos = 0;
    while (pos < padded.size()) {
      for (i in blk.keys()) { blk[i] := padded[pos + i] };
      pos += 64;
      var wi = 0;
      while (wi <= 15) {
      ...
      var a = h0v; var b = h1v; var c = h2v; var d = h3v;
      var e = h4v; var f = h5v; var g = h6v; var h = h7v;
      i := 0;
      while (i <= 63) {
      ...
      h0v := add32(h0v, a); h1v := add32(h1v, b); h2v := add32(h2v, c); h3v := add32(h3v, d);
      h4v := add32(h4v, e); h5v := add32(h5v, f); h6v := add32(h6v, g); h7v := add32(h7v, h);
    };
    Array.concat(
      Array.concat(
        Array.concat(nat32ToBe32(h0v), nat32ToBe32(h1v)),
        Array.concat(nat32ToBe32(h2v), nat32ToBe32(h3v)),
      ),
      Array.concat(
        Array.concat(nat32ToBe32(h4v), nat32ToBe32(h5v)),
  ...
  public func sha256Hex(data: Blob) : Text {
    Hex.encode(sha256(Blob.toArray(data)))
  };
};
